<template>
    <div class="body">
        <div>
            <ul>
                <li>
                    <span class="show_ipt">注册</span>
                    <span @click="$emit('change')">登录</span>
                </li>
                <li>账号：<input type="text" v-model="id" /></li>
                <li>密码：<input type="password" v-model="password" /></li>
                <li>
                    再次确认：<input type="password" v-model="password_a" />
                </li>
                <li @click="set()">确认注册</li>
            </ul>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        set() {
            if (this.id == "") {
                alert("账号不能为空");
            } else if (this.password == "" || this.password_a == "") {
                alert("密码不能为空");
            } else if (this.password != this.password_a) {
                alert("请再次输入密码");
            } else {
                console.log(this.id, this.password);
                fetch(`/register?userName=${this.id}&password=${this.password}`)
                    .then((r) => r.json())
                    .then((res) => {
                        if (res.code == 1) {
                            alert("注册成功");
